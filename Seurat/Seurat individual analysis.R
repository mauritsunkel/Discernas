...
# selection_panel <- c("MAP2", "RBFOX3", "NEUROG2")
# # define panel type
...
selection_percent_expressed <- 25
...
# TODO change creating dir code to how it's done in integrated (looks more clear, and less code)
...
data <- SCTransform(data, method = "glmGamPoi", vars.to.regress = "percent.mt", verbose = T, variable.features.n = n_var_features)
...
if (run_label_transfer) {
  ...
  p <- DimPlot(data, reduction = "umap", label = FALSE, pt.size = 0.5, group.by = 'SingleR.cell.first.labels', repel = TRUE)
  ggsave(file=paste0("Label_transfer/cell_first.labels_UMAP_", sample_name, ".png"), width = 30, height = 20, units = "cm")
  p <- DimPlot(data, reduction = "umap", label = TRUE, pt.size = 0.5, repel = TRUE) # + NoLegend()
  ...
  p <- DimPlot(data, reduction = "umap", label = FALSE, pt.size = 0.5, group.by = 'SingleR.cell.labels', repel = TRUE)
...
# save procesed data
saveRDS(data, file = paste0(sample_name, ".rds")) # neuronal, astrocytical or mixed

## save subset - selection - data
# create dotplot to extract percent expressed (and average expression) data
p <- DotPlot(data, features = selection_panel)
...
subset <- subset(data, idents = cluster_selection)
# add selection panel and type to subset file as metadata (for later reference)
subset@misc$selection_panel <- as.data.frame(selection_panel, replicate(length(selection_panel), selection_panel_type))
# save subset RDS file
saveRDS(subset, file = paste0(sample_name, "_", selection_panel_type, "_subset.rds"))
...
# BL_A_old <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/Exploration results/SCTransform + Leiden - Cellcycle/BL_A/astrocytical-subset.rds")
# BL_N_old <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/Exploration results/SCTransform + Leiden - Cellcycle/BL_N/neuronal-subset.rds")
# BL_C_old_neuron <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/Exploration results/SCTransform + Leiden - Cellcycle/BL_C/mixed-neuronal-subset.rds")
# BL_C_old_astrocyte <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/Exploration results/SCTransform + Leiden - Cellcycle/BL_C/mixed-astrocytical-subset.rds")
# BL_A_new <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/SCTransform + Leiden - Cellcycle + Annotation/BL_A/BL_A_astrocytical_subset.rds")
# BL_N_new <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/SCTransform + Leiden - Cellcycle + Annotation/BL_N/BL_N_neuronal_subset.rds")
# BL_C_new_neuron <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/SCTransform + Leiden - Cellcycle + Annotation/BL_C/BL_C_neuronal_subset.rds")
# BL_C_new_astrocyte <- readRDS("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/SCTransform + Leiden - Cellcycle + Annotation/BL_C/BL_C_astrocytical_subset.rds")
# levels(Seurat::Idents(BL_A_old))
# levels(Seurat::Idents(BL_A_new))
# levels(Seurat::Idents(BL_N_old))
# levels(Seurat::Idents(BL_N_new))
# levels(Seurat::Idents(BL_C_old_neuron))
# levels(Seurat::Idents(BL_C_new_neuron))
# levels(Seurat::Idents(BL_C_old_astrocyte))
# levels(Seurat::Idents(BL_C_new_astrocyte))
...
######################################################
### CIPR: Cluster Identity Predictor (Seurat --> CIPR)
# library(CIPR)
# work_dir <- "C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/results/Exploration results/SCTransform + Leiden - Cellcycle/BL_C"
# setwd(work_dir)
#
# # saved earlier, after Seurat::FindAllMarkers and before DEG analysis
# data.markers <- read.csv2(file = "DEG-analysis_genes-list.csv")
#
# ref <- "mmrnaseq" # hpca (Human Primary Cell Atlas) or mmrnaseq (various tissues) for our dataset
# CIPR::CIPR(input_dat = data.markers,
#      comp_method = "logfc_dot_product", # recommended by CIPR
#      plot_ind = F,
#      plot_top = T,
...
#      reference = ref,
#      if (ref == "hpca") {
#        select_ref_subsets = c("Astrocyte", "Embryonic stem cell", "GMP", "MEP", "Endothelial cell", "Epithelial cell",
#                               "HSC", "MSC", "iPS cell", "Tissue stem cell", "Fibroblast", "Neuroepithelial cell", "DC",
#                               "Gametocyte", "Neurons")
#      } else {
#        select_ref_subsets = c("Astrocyte", "Dendritic cell", "Endothelial cell", "Epithelial cell", "Fibroblast", "Microglia",
...
#      save_png = TRUE
# )
# dir.create(paste0("CIPR/", ref))
# for (i in seq_along(ind_clu_plots)) {
#   png(filename=paste0("CIPR/", ref, "/", names(ind_clu_plots)[i], ".png"), width = 1600)
#   plot(ind_clu_plots[[i]])
#   dev.off()
# }
# png(filename=paste0("CIPR/", ref, "/top_plots.png"), width = 1600)
# plot(top_plots)
# dev.off()
########################### END CIPR #############################
