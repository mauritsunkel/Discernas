...
rds.files <- c("C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/Seurat/results/Pipe +SCT +Leiden -Cellcycle +SingleR +Autoselection/BL_N/BL_N.rds",
               "C:/Users/mauri/Desktop/M/Work & Education/Erasmus MC PhD/Projects/Single Cell RNA Sequencing/Seurat/Seurat/results/Pipe +SCT +Leiden -Cellcycle +SingleR +Autoselection/BL_C/BL_C.rds")
...
integration_analysis <- function(integrated, selection_performed = FALSE) {
  ...
  if (run_label_transfer) {
    ...
    SingleR::plotScoreHeatmap(integrated.sce.labels)
    dev.off()
    png(paste0("Label_transfer/cluster_heatmap_scores=first_labels=tuned_", sample_name, ".png"))
    SingleR::plotScoreHeatmap(integrated.sce.clusters)
    dev.off()
